    ...
    public static class ApplicationContext
    {
        ...
        public static T? GetSessionData<T>(string key) where T : class
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(sValue))
                {
                    return System.Text.Json.JsonSerializer.Deserialize<T>(sValue);
                }
            }
            catch
            {
        ...
        /// <summary>
        /// Số dòng hiển thị trên mỗi trang (mặc định lấy từ cấu hình "PageSize")
        /// </summary>
        public static int PageSize
        {
            get
            {
                var val = GetConfigValue("PageSize");
                return int.TryParse(val, out int result) ? result : 20;
            }
        }
    }
}
